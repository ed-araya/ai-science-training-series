...
### EA: 3. Loading data according to rank ID and ajusting the number of time steps
...
#opt = tf.optimizers.Adam(args.lr) #EA: commented
#### EA: 4. Scale the learning rate with number of workers
#opt = tf.train.AdagradOptimizer(0.01*hvd.size()) #line as given in the instructions, but error
...
def training_step(images, labels):
    ...
    ### EA ###
    # HVD-5 Wrap tf.GradientTape with Horovod Distributed Gradient Tape
...
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        ...
        ### EA: 6. Broadcast the model from rank 0
        hvd.broadcast_variables(mnist_model.variables, root_rank=0)
        hvd.broadcast_variables(opt.variables(), root_rank=0)
        ### Unclear: Note: broadcast should be done AFTER the first gradient step to ensure optimizer initialization.
        ######

        ### EA: 7. Checkpointing on root rank
        if hvd.rank() == 0:
            checkpoint.save(checkpoint_dir)
        ####

        ### EA: 8. Average the metrics across all the workers
        #loss_value = hvd.allreduce(loss, average=True)
