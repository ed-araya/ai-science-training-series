...
print('\n\n EA: current parallel_threads = ',parallel_threads)
# This controls how many batches to prefetch
prefetch_buffer_size = 8 # tf.data.AUTOTUNE
print(' EA: current prefetch_buffer_size = ', prefetch_buffer_size)
...
def train_epoch(i_epoch, step_in_epoch, train_ds, val_ds, network, optimizer, BATCH_SIZE, checkpoint):
    ...
    output_file.write('iteration loss accuracy images_per_second\n')
    output_file.close()
    for train_images, train_labels in train_ds.take(steps_per_epoch):
        ...
        output_file.write(f"{step_in_epoch.numpy()} {loss:.3f} {acc:.3f} {images_per_second:.3f} \n")
        output_file.close()
