...
#
## Modified version using .npz
    np.savez_compressed(
        "tensor_dataset.npz",
        src=src.numpy(), # Pass the NumPy array with 'src' as the key
        tgt=tgt.numpy()  # Pass the NumPy array with 'tgt' as the key
    )
    print("Data saved successfully to tensor_dataset.npz")
...
#
## EA: load the .npz tensor instead:
# Custom Dataset to load data from a NumPy compressed archive (.npz)
class NPZTensorDataset(torch.utils.data.Dataset):
    def __init__(self, npz_file_path):
        # Load all data from the .npz file into memory upon initialization
        # np.load returns a dictionary-like object
        data = np.load(npz_file_path)
        
        # Access the arrays by their saved names ("src" and "tgt")
        self.src = data["src"]
        self.tgt = data["tgt"]
        
        # Note: We do not need a 'close' method as the file handle is closed
        # automatically after np.load() finishes reading the data into memory.

    def __len__(self):
        # Return the number of samples (assuming src and tgt have same length)
        return len(self.src)

    def __getitem__(self, idx):
        # Get the src and tgt arrays for the given index
        # Convert the numpy arrays to PyTorch tensors within __getitem__
        src_tensor = torch.tensor(self.src[idx], dtype=torch.float32)
        tgt_tensor = torch.tensor(self.tgt[idx], dtype=torch.float32)
        return src_tensor, tgt_tensor

# Load the dataset
# Make sure "tensor_dataset.npz" exists from the previous steps
#dataset = NPZTensorDataset("tensor_dataset.npz")
